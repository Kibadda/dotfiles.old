...
local stylua_finder = function(path)
  if cached_config[path] == nil then
    ...
    for index, dir in ipairs(file_parents) do
      ...
      if stylua_path:exists() then
        cached_config[path] = stylua_path:absolute()
        break
      end
  ...
  return cached_config[path]
end

local stylua = {}

function stylua.format(bufnr)
  ...
  if j.code ~= 0 then
    vim.schedule(function()
      vim.notify "[stylua] Failed to process due to errors"
    end)
